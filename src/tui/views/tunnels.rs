...
use ratatui::prelude::*;
use ratatui::widgets::{Block, Borders, Paragraph, Row, Table, Cell, Gauge, Padding};
...
pub fn render_state(frame: &mut Frame, state: &RenderState, area: Rect) {
    ...
    let empty_text = vec![
        Line::from(""),
        Line::from(vec![
            Span::styled("  No tunnels configured", theme.text_dim()),
        ]),
        Line::from(""),
        Line::from(vec![
...
pub fn render(frame: &mut Frame, app: &App, area: Rect) {
    ...
    // Sample tunnel data
    let rows = vec![
        create_tunnel_row("●", "DB Forward", "Local", "localhost:3306 → db.prod:3306", "12.4 MB", "45.2 MB", "3", theme, true),
        create_tunnel_row("●", "Web Proxy", "Dynamic", "localhost:1080 (SOCKS5)", "1.2 GB", "3.4 GB", "12", theme, false),
        create_tunnel_row("○", "Remote SSH", "Remote", "remote:2222 → localhost:22", "--", "--", "0", theme, false),
    ];
    ...
    if rows.is_empty() {
        let empty_text = vec![
            ...
            Line::from(""),
            Line::from(vec![
                Span::styled("  Tunnel Types:", Style::default().add_modifier(Modifier::BOLD).fg(theme.fg_bright())),
            ]),
            Line::from(vec![
