...
fn test_host_tagging() {
    ...
    let mut host = HostConfig::new("server", "host.com", "user");
    host.tags = vec!["production".to_string(), "web".to_string(), "frontend".to_string()];
...
fn test_environment_config() {
    ...
    assert_eq!(host.environment.get("TERM"), Some(&"xterm-256color".to_string()));
}
